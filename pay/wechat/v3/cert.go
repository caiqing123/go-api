...
func (c *ClientV3) GetAndSelectNewestCert() (cert, serialNo string, err error) {
	...
	if certs.Code == Success && len(certs.Certs) > 0 {
		...
		if len(certs.Certs) == 1 {
			...
			if err != nil {
				return pay.NULL, pay.NULL, fmt.Errorf("time.ParseInLocation(%s, %s),err:%w", xtime.TimeLayout, formatExpire, err)
			}
			if time.Now().Unix() >= expireTime.Unix() {
				// 过期了
				return pay.NULL, pay.NULL, fmt.Errorf("wechat platform API cert expired, expired time: %s", formatExpire)
			}
		...
		for _, v := range certs.Certs {
			...
			if err != nil {
				return pay.NULL, pay.NULL, fmt.Errorf("time.ParseInLocation(%s, %s),err:%w", xtime.TimeLayout, formatEffective, err)
			}
	...
	// failed
	return pay.NULL, pay.NULL, fmt.Errorf("GetPlatformCerts() failed or certs is nil: %+v", certs)
}
